...
def main():
    ...
    try:
        with GitHubMCPClientSync(token) as client:
            # Close completed Phase 1 & 2 Cleanup tasks
            tasks_to_close = [
                (1, "Completed: Platform is stable and deployable via docker-compose with updated verification scripts."),
                (2, "Completed: Implemented Redis-based session persistence, context propagation, and ID tracking."),
                (3, "Completed: Orchestrator is now context-aware, passing session metadata to sub-agents."),
                (4, "Completed: Session cleanup logic implemented in Redis Session Service."),
                (5, "Completed: Removed direct GCP CLI/SDK calls (including BigQuery Plugin). Refactored agents to use MCP tools and ADK patterns.")
            ]

            for issue_id, comment in tasks_to_close:
                close_issue(client, issue_id, comment)
