...
from tools import (
    ...
    puppeteer_fill, puppeteer_evaluate, puppeteer_hover
)
from mcp_client import PuppeteerMCPClient, _mcp_ctx
...
async def send_message_async(prompt: str, user_email: str = None, project_id: str = None, session_id: str = "default") -> str:
    ...
    if RedisEventPublisher:
         try:
             publisher = RedisEventPublisher("Puppeteer Agent", "Automation Specialist")
             _redis_publisher_ctx.set(publisher)
         except Exception as e:
             logger.warning(f"Failed to initialize RedisEventPublisher: {e}")

    # Publish "Processing" event
    await _report_progress(f"Automating Browser...", icon="🎭", display_type="toast")
