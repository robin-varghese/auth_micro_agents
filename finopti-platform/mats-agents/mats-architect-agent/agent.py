...
def create_architect_agent(model_name: str = None) -> Agent:
    ...
    logger.info(f"Creating Architect agent with model: {model_to_use}")
...
async def process_request(prompt_or_payload: Any, session_id: str = None, user_email: str = None, auth_token: str = None):
    ...
    if isinstance(prompt_or_payload, dict):
        ...
        try:
            ...
            pass  # prompt stays as-is; not a JSON payload
    ...
    if auth_token:
        try:
            ...
            logger.warning(f"[{session_id}] Architect: _auth_token_ctx not available in context module")
        os.environ["CLOUDSDK_AUTH_ACCESS_TOKEN"] = auth_token

    current_user_email = user_email or _user_email_ctx.get() or "unknown"

    logger.info(
        f"[{session_id}] Architect: process_request started | "
        f"user={current_user_email} | prompt_length={len(prompt)} chars | "
        ...
        if RedisEventPublisher:
            ...
            if session_id:
                ...
                pub.publish_event(
                    ...
                    msg_type="STATUS_UPDATE", message="Architect starting RCA synthesis...",
                    display_type="step_progress", icon="🏗️"
                )
                logger.info(f"[{session_id}] Architect: Redis publisher initialized and notified")
    except Exception as redis_err:
        logger.warning(
            f"[{session_id}] Architect: Redis publisher initialization failed — "
    ...
    with tracer.start_as_current_span(
        ...
        **span_context,
        ...
        if session_id and span and span.is_recording():
            ...
            logger.info(f"[{session_id}] Architect: Set session.id on OTel span")
        ...
        def _create_app(model_name: str):
            logger.info(f"[{session_id}] Architect: Creating app with model={model_name}")
        ...
        async def _run_once(app_instance):
            ...
            try:
                async with InMemoryRunner(app=app_instance) as runner:
                    ...
                    await runner.session_service.create_session(session_id=sid, user_id=uid, app_name="mats_architect_app")
                    msg = types.Content(parts=[types.Part(text=prompt)])
                    
                    logger.info(f"[{session_id}] Architect: InMemoryRunner started, dispatching to LLM")
                    event_count = 0
                        ...
                        if hasattr(event, 'content') and event.content:
                            for part in event.content.parts:
                                if part.text:
                                    ...
                                    current_sid = _session_id_ctx.get()
                                    if pub and current_sid:
                                        try:
                                            pub.publish_event(
                                                session_id=current_sid, user_id=current_user_email, trace_id="unknown",
                                                msg_type="THOUGHT", message=part.text[:200],
                                                display_type="markdown", icon="🧠"
                                            )
                                        except Exception as pub_err:
                                            logger.warning(
                                                f"[{session_id}] Architect: Failed to publish THOUGHT event | error={pub_err}"
                                            )

                    elapsed = time.monotonic() - runner_start
                    logger.info(
                        f"[{session_id}] Architect: LLM run completed | "
                ...
                err_msg = str(e)
                logger.error(
                    f"[{session_id}] Architect: InMemoryRunner failed after {elapsed:.1f}s | "
                ...
                error_response = json.dumps({
                    "status": "FAILURE",
                    "confidence": 0.0,
                    "rca_content": f"Architect internal error ({type(e).__name__}): {err_msg}",
                    "limitations": ["Agent internal error during synthesis"]
                })
                response_text = error_response
                if "429" in err_msg or "Resource exhausted" in err_msg:
                    logger.warning(f"[{session_id}] Architect: Quota exhausted (429), will trigger model fallback")
                    raise e
            
             # Check for soft 429 in response text
            if "429 Too Many Requests" in response_text or "Resource exhausted" in response_text:
                 logger.warning(f"[{session_id}] Architect: Soft 429 detected in response text, raising for model fallback")
        ...
        logger.info(
            f"[{session_id}] Architect: process_request complete | "
